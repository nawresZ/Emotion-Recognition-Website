...
def uploadimage():
    ...
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            return render_template('Visual.html', orig=result[0], pred=result[1], bar=result[2], music=result[3], image=result[3])
                                   #sentence=sentence, activity=activity, link=link)
